...
#
# Edit scripts/scenarios.json to add/change/remove scenarios - no code changes needed here.
# Runs smoke-test.sh first as a pre-flight check (pass --skip-smoke to bypass, not
...
# `exit` inside one bench-*.sh only ends that scenario, never this orchestrator.
#
# Usage:
