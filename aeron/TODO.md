...
**#5（PTP前提の実サーバー間片道レイテンシ）は実装完了。** 対象2台がDockerを使えないため、
ネイティブ実行経路（`scripts/common-native.sh`・`lib/aeron-driver.sh`）と配布用tarball
（`scripts/package-native.sh`）を含めて実装した。実機2台での実行のみ残っている。

優先順位: #1 → #2 → #3 → #4 → #6（#5は実機実行待ち、#1と同時に行うのが効率的）
...
**状態**: ✅ **実装済み**（実機2台での実行は未実施 — #1と同時に行うこと）

対象の2台が**Dockerを使えない**ことが判明したため、ネイティブ実行経路ごと実装した。

実装したもの:

- **`aeron_bench --clock monotonic|realtime`**。`Clock`（区間計測用の`CLOCK_MONOTONIC`）は
  据え置き、**ワイヤに載る時刻だけ**を`wireNowNs()`として分離した。単調時計を丸ごと
  差し替えるのは誤った直し方（区間計測がstepし得る時計になる）。`environment.clock`に記録。
  `--measure rtt`／`throughput` では明示的にエラーにする（詳細は`CLAUDE.md`）。
- **負のレイテンシ検出**（`warnIfClockLooksUnsynchronised`）。同期が壊れている場合の
  **唯一の自動的な兆候**。pub側`realtime`／sub側`monotonic`で意図的にずらして検証済み
  （2000/2000件を負と判定し、原因の時計まで指摘した）。
- **`scripts/bench-oneway-2host.sh`** — `--role pub|sub`。計測結果は**sub側**に出る
  （RTT版とは逆）。`ethtool -T`でハードウェアタイムスタンプを確認し、`pmc`で
  `master_offset`を実行前後に取得して`meta.json`の`ptp`に記録する。
- **ネイティブ実行経路** `scripts/common-native.sh` と、ドライバのライフサイクルを
  コンテナ版と共有するための `lib/aeron-driver.sh`。
- **`scripts/package-native.sh`** — 約1MBの自己完結tarball。移植性は実測で確認
  （Ubuntu 22.04 / glibc 2.35 のクリーン環境に展開し、2コンテナ間でpub/subを
  欠落0で完走）。

**実機2台で残っている作業**:

1. `preflight.sh` を両サーバーで実行し、`ethtool -T` がハードウェアタイムスタンプを
   報告することを確認する。**ソフトウェアタイムスタンプしか無い場合は片道測定を諦めて
   `bench-rtt-2host.sh` に戻ること** — 誤差が測定対象と同じ桁になる。
2. `sockperf` / `netperf -t UDP_RR` でネットワーク単独の下限を先に取る（数分、かつ決定的）。
3. 片道を**両方向**測り、RTTと突き合わせる。**非対称分が時計オフセット誤差の読み取り値**。
4. `bench-rtt-2host.sh` も実機2台では未検証（同一ホスト上のhostネットワークコンテナ2つでは
   確認済み）。

**20コアでの注意**（#1にも関連）:

- 2台片道の1対1構成は pub側4コア（ドライバ3 + ペーシング1）、sub側4コア
  （ドライバ3 + poll 1）。20コアなら余裕があり、`--driver-idle noop` と
  `--poll-idle busy` を両方使ったまま測れる。WSL2で実効884/sしか出なかった
  スピナ競合の交絡は消える見込み。
- **スケーラビリティ掃引は`--sub-counts 1,5,10,15`にすること。** 「コア数 ≥ subscriber数 + 4」
  が15まで成立する（15 + 3 + 1 = 19 ≤ 20）。**既定の25は20コアでも収まらず**、
  WSL2と同じCPU枯渇のアーティファクトを再生産する。
- 2ソケット×10構成ならNUMAを確認し、NICのローカルノード側にドライバスレッドを寄せること。
