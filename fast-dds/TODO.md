...
**状態**: ✅ 実装済み（`scripts/bench-oneway-2host-native.sh`、PTP前提）
...
- それでも実配置での確認が要る場合は `bench-rtt-2host.sh`（RTT、時計同期不要）を使う。
...
**片道対応の実装**:
- `--clock realtime|monotonic` を追加し、`CLOCK_REALTIME` ベースで計測できるようにした。
